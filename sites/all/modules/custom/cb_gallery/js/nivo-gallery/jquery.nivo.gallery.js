 ...
 *
 * CHANGE LOG:
 * Dean Shelton : 3/25/2012
 * 
 * Description of changes:
 * For the CB Gallery a few modifications were made to this plugin.
 * 1. Event Bindings were rewritten to adhere to new jQuery Code standards.
 * 2. plugin.init() method call was moved above the event bindings.
 */
...
(function($) {
    ...
    $.nivoGallery = function(element, options){
        ...
        // DS -
        // This method was being called too late. I moved it up to be called
        // above the event bindings, as the plugin.init() calls the local method
        // setupGallery() and in-turn CREATES the markup that is to be bound.
        // I honestly do not know how this was supposed to work when calling
        // plugin.init() at the bottom... wtf.
        plugin.init(); 
        /*
         DS -
            ...
            Reading this code, however, i can see that this is simply binding
            the '.click()' event. It makes me wonder why we do not just use '.click()'
            I made the descision to rewrite these bindings with jQuery.click() as it has been used
            since jQuery 1.0, and continues to be supported through the current
            jQuery (v1.7).
        ...
        $element.find('.nivoGallery-play').click(function(){
            $(this).toggleClass('playing');
            global.paused = !global.paused;
            runTimeout();
            return false;
        ...
        $element.find('.nivoGallery-prev').click(function(){
            plugin.prevSlide();
         });
        
        $element.find('.nivoGallery-next').click(function(){
            plugin.nextSlide();
        });
        $element.find('.nivoGallery-fullscreen').click(function(){
            $element.toggleClass('fullscreen');
        });
